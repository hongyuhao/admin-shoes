...
public class PageTurn implements Serializable {
	...
	private Integer startIndex = new Integer(0);//开始记录
	 ...
	 *            每页显示的条数
	 */
	public PageTurn(int startIndex, int page, int pagesize, int flag) {
		this.startIndex = startIndex;
		initWithStart(startIndex + (page - 1) * pagesize, pagesize, (page > 0 ? page : 1) * pagesize);
	}
	 ...
	 *            总的记录条数
	 */
	public void initWithPage(int startIndex, int page, int pagesize, int rowscount) {
		this.startIndex = startIndex;
		initWithStart(startIndex+(page - 1) * pagesize, pagesize, rowscount);
	}
	...
	public void initWithStart(int startRow, int pagesize, int rowscount) {
		...
		if (startResult - this.startIndex > rowscount) {
