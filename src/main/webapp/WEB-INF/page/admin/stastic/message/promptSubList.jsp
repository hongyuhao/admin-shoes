...
<form id="pagerForm" onsubmit="return divSearch(this, '${targetTab}');" action="/admin/stastic/messagePromptInDay/subList?targetTab=${targetTab }" method="post">
...
<div class="pageContent" style="border-left:1px #B8D0D6 solid;border-right:1px #B8D0D6 solid">
	 <table class="table" width="100%" layoutH="425" rel="${targetTab }">
		...
		<tbody>
			<c:choose>
				<c:when test="${null==messagePromptInDayList||messagePromptInDayList.isEmpty()}">
					<tr class="tatr2">
						<td colspan="11" align="center">暂无数据</td>
					</tr>
				</c:when>
				<c:otherwise>
					<c:forEach var="message" items="${messagePromptInDayList }">
						<tr>
...
<script>
	$.ajax({
		type:'get',
		url:'/admin/stastic/messagePromptInDay/chart.json',
		dataType:'json',
		data: {
			start:'${start}',
			end:'${end}'
