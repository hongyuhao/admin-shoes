...
<form id="pagerForm" method="post" action="${path}/admin/ucenter/integralPromotionGoodsRelation/promotionList?sendCardType=3&showType=2">
		  			...
		  			<c:choose>
						...
						<c:otherwise>
							<c:forEach items="${promotionList}" var="card">
								...
								<td>${card.getLimit}</td>
								<td><a class="btnSelect" href="javascript:$.bringBack({relationId:'${card.id}',relationName:'${card.title}'})"
                       				title="查找带回">选择</a></td>
								</tr>
							</c:forEach>
						</c:otherwise>
